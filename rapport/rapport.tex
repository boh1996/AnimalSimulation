...
\section{Introduktion} \label{sec:introduction}
  ...
  \paragraph*{Sådan kompilerer du projektet\\}
    ...
    Unit-test findes i  mappen \path{/src}, i filen \path{test.fsx}.

    Flere informationer omkring settings-filerne kan findes i \path{readme.md}
...
\section{Problemformulering} \label{sec:problem}
  ...
  \section{Problemanalyse og design} \label{sec:design}
    ...
    \subsection*{HistoryRecord}
      \texttt{HistoryRecord} er en container klasse, der indeholder tilstanden 
      for habitatet per tick; så en \texttt{HistoryRecord} indeholder 
      informationer om habitatet for et tick.
      Den skal indeholde antal ticks siden start, antal byttedyr og antal 
      rovdyr ved dette tick.
      Klassen har en metode \textit{toJSON}, som objektet til et JavaScript 
      Objekt, serialiseret.
  ...
  \section{Programbeskrivelse} \label{sec:programDescription}
    ...
    \subsection*{Settings}
      ...
      Med programmet følger en default indstilling, hvor rovdyr og byttedyr har 
      lige stor chance for overlevelse, samt 2 andre indstillinger hvor de hver 
      især har overtaget.
      default filen kan ses i \namedref{lst:defaultSettings}.
  ...
  \section{Afprøvning} \label{sec:unitTest}
    ...
    For at sikre at vores output fil følger de rette standarder, har vi kørt den gennem \href{http://jsonlint.com}{JSONLint}.
    \begin{figure}[H]
      \centering
      \includegraphics[width=400px]{figures/JSONLit.jpg}
      \caption{JSONLint resultat}
      \label{fig:jsonLINT}
    \end{figure}

	\section{Diskussion og konlusion} \label{sec:conclusion}
    Vi har i dette projektet designet og bygget en simulator,
    til at simulere hvordan to grupper af dyr "Prey" og "Predators" fungerer i et simpelt habitat.
    Simulatoren er designet og udviklet efter OOP paradigmet, og udnytter klasser og nedarvning.
    Programmet lever op til de krav der er stillet til programmet i \namedref{ssec:demands}.
    Simulationen følger de stillede regler beskrevet under reglerne i \namedref{ssec:demands}.
  ...
  \section{Bilag}
      \subsection{Brugervejledning} \label{ssec:manual}
        Programmet kan kompileres med Make-filen der kan findes i \path{/src} mappen.
        For at køre programmet, kør \path{simulation.exe}.
        Programmet vil derefter spørge om en relativ sti (fra kørsels mappen) til den settings fil du vil bruge.
        De medfølgende settingsfiler kan findes i \path{/settings} mappen.

        Når simulatoren har kørt, laver den en ".json" fil, der ligges i \path{/output} mappen.
